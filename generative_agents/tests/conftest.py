"""conftest — story_weaver 測試 fixtures."""
...
def _make_template_dir(root: str, name: str, living_area: list, with_texture: bool = True):
...
def agents_root(tmp_path):
    ...
    for name, home in FAKE_TEMPLATES.items():
        _make_template_dir(str(root), name, home)
    return str(root)
